...
/**
 * Promisified implementation of 7zip-min unpack
 * @param {string} source - .7z file to decompress
 * @param {string} out - output directory
 * @returns {Promise<string>}
 */
function un7zip(source, out) {
  return new Promise((resolve, reject) => {
    _7zip.unpack(source, out, (err) => {
      if (err) {
        reject(err);
      }
      resolve();
...
async function deploy({ name, spaArchive, appPath, ref } = {}) {
  ...
  // if the archive is 7zipped, extract with 7zip (native, faster), otherwise use decompress
  if (/\.7z$/.test(spaArchive)) {
    await un7zip(spaArchive, tmpDir);
  } else {
