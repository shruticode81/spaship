    ...
    if (configContent.token && configContent.server) {
      ...
      if (!isExpired) {
        if (configContent.file && configContent.dist) {
          ...
          this.error(
            chalk.redBright(".spashipsessionrc.yaml file does'nt consist .spaship file path and dist path ") +
              chalk.cyanBright(",Try running spaship init Command")
          );
        }
      } else {
...
`;
UploadCommand.examples = [`$ spaship upload`];
